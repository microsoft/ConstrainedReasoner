...
class hallucination_mitigation_prompt :
    ...
    def __init__(self, use_chat_completions : bool, prompt_resource_root_folder : str = None, inputreason = True, outputreason= False) -> None:
        ...
        if inputreason:
            ...
            self._prompt = self._loadPrompt('hallucination_mitigation/v3_noReason_OutputReason', use_chat_completions)
        else:
            self._prompt = self._loadPrompt('hallucination_mitigation/v3_noReason', use_chat_completions)
    ...
    # The parameter items is a list of dicts with keys: hypothesis, enc_id, sentence_id etc.
    # Here the hypothesis in fact is the whole sentence with entity name highlighted
    def create_prompt(self, transcript: str, rewrite_instructions: str, max_tokens: int) -> str:
...
class hallucination_reasoning_prompt :
    def __init__(self, use_chat_completions : bool, prompt_resource_root_folder : str = None, category = False) -> None:
        self._tokenizer = GPT2TokenizerFast.from_pretrained("gpt2")
        self._prompt_resource_root = prompt_resource_root_folder if prompt_resource_root_folder is not None else os.path.join(os.path.dirname(__file__), "..")
        self._maxPromptTokens = 32000
        self._use_chat_completions = use_chat_completions
